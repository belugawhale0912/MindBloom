...
export default function WhiteNoiseMixer() {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 ease-out pb-12 space-y-8">
      {/* Page Header */}
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <Button
          variant="secondary"
          className="rounded-full h-10 px-5 text-sm font-semibold shadow-sm"
          onClick={handleRandomPlay}
        >
          <Play className="h-4 w-4 mr-2 text-primary" /> Random Mix
        </Button>
      </div>
      ...
      {savedMixes.length > 0 && (
        <div className="space-y-3 animate-in fade-in slide-in-from-top-2">
          <span className="text-sm font-semibold text-muted-foreground">Your Saved Mixes</span>
          <div className="flex flex-wrap gap-2">
            {savedMixes.map((mix) => (
              <div key={mix.id} className="flex items-center rounded-full bg-secondary/30 border border-border/50 pr-1 hover:border-primary/30 transition-colors shadow-sm">
                <Button
                  variant="ghost"
                  size="sm"
                  ...
                  onClick={() => loadMix(mix)}
                >
                  <Play className="h-3 w-3 mr-2 text-primary" /> {mix.name}
                </Button>
                <Button
                  variant="ghost"
                  size="icon"
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="w-full lg:w-80 shrink-0">
          <div className="sticky top-24 flex flex-col gap-6">
            <Card className="border-0 shadow-sm ring-1 ring-border/50 rounded-3xl bg-secondary/10 overflow-hidden">
              <CardContent className="p-6 space-y-6">
                ...
                <div className="flex flex-col gap-3">
                  <Button
                    size="lg"
                    className="w-full rounded-full h-14 text-base shadow-sm font-bold bg-primary hover:bg-primary/90 transition-all"
                  ...
                  >
                    <Square className="mr-2 h-4 w-4" /> Clear All
                  </Button>
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-mono text-primary bg-primary/10 rounded-full px-2 py-0.5">
                      {timerLeft > 0 ? formatTime(timerLeft) : "--:--"}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {[10, 30, 60].map(m => (
                      <Button
                        ...
                        variant="secondary"
                        size="sm"
                        className={`flex-1 rounded-full text-xs h-8 transition-colors ${timerLeft === m * 60 ? "bg-primary text-white hover:bg-primary/90" : "hover:bg-secondary/80"}`}
                  ...
                  {timerLeft > 0 && (
                    <Button
                      variant="ghost"
                      size="sm"
                      className="w-full rounded-full text-xs h-8 text-muted-foreground mt-1"
                      onClick={clearTimerAndPause}
                    >
                      <X className="h-3 w-3 mr-1" /> Clear Timer
                    </Button>
                  )}
                </div>
                ...
                <div className="space-y-3 pt-4 border-t border-border/50">
                  <span className="text-sm font-semibold">Save Current Mix</span>
                  <Input
                    placeholder="Name this mix..."
                    value={mixName}
                    onChange={(e) => setMixName(e.target.value)}
                    disabled={isSaving}
                    className="rounded-full border-border/80 focus-visible:ring-primary/30 h-10"
                  />
                  <Button
                    variant="default"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {SOUNDS.map((sound) => {
              ...
              return (
                <Card
                  key={sound.id}
                  className={`border transition-all duration-300 overflow-hidden rounded-3xl ${
                    isActive
                      ? `${sound.activeBg} ${sound.activeBorder} shadow-sm ring-1 ring-${sound.color.split("-")[1]}-500/20 scale-[1.02]`
                      : "border-border/50 shadow-none hover:border-border hover:bg-secondary/20"
                  }`}
                >
                  <CardContent className="p-5 flex flex-col items-center justify-center text-center h-full gap-3 relative">
                    <button
                    ...
                    >
                      <Icon className={`h-7 w-7 ${isActive ? sound.color : ""}`} />
                    </button>

                    <h3
                      className={`text-sm font-semibold transition-colors ${isActive ? "text-foreground" : "text-muted-foreground"}`}
                    >
                      {sound.label}
                    </h3>

                    <div
                      className={`w-full transition-all duration-300 ease-in-out ${isActive ? "opacity-100 h-8 mt-1" : "opacity-0 h-0 overflow-hidden mt-0"}`}
                    >
                      <Slider
